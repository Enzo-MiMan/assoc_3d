...
class Ransac_Data_Loader():
    ...
    def __getitem__(self, index):
        ...
        # read image
        img_file_dst = join(self.sequence_path, 'enzo_ransac_depth_image_dst', str(timestamp_dst)+'.png')
        img_file_src = join(self.sequence_path, 'enzo_ransac_depth_image_src', str(timestamp_src)+'.png')
        image_dst_org = cv2.imread(img_file_dst, cv2.COLOR_BGR2GRAY)
        image_src_org = cv2.imread(img_file_src, cv2.COLOR_BGR2GRAY)
